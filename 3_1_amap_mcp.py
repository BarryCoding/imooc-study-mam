...
# 定义高德mcp服务基础地址
AMAP_URL = f"https://mcp.amap.com/mcp?key={os.getenv('AMAP_KEY')}"
...
class ReActAgent:
    ...
    def init_amap_mcp(self) -> None:
        """初始化高德mcp服务"""
        # 1.调用高德mcp服务获取工具列表信息
        tools_list_response = requests.post(
            AMAP_URL,
    ...
    def call_amap_mcp(self, name: str, arguments: dict) -> str:
        """调用高德mcp服务"""
        # 1.发起请求调用高德mcp服务
        tools_call_response = requests.post(
            AMAP_URL,
            headers=self.headers,
            json={
                "jsonrpc": "2.0",
    ...
    def process_query(self, query: str = "") -> None:
        ...
        if is_tool_calls:
            ...
            for tool_call in tool_calls_json:
                ...
                # 调用工具
                try:
                    result = self.call_amap_mcp(tool_name, tool_arguments)
...
# user prompt example1: 你好, 你是?
# user prompt example2: 帮我查一下武汉的天气
